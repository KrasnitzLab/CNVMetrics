...
For this demonstration, we consider _CNV calls_ as obtained with 
...
## The CNV calls for all samples are present in one file
...
the similarity metrics. By default, an hierarchical clustering based on
based on the sample distances (1-metrics) is used.

```{r demoPlot01, fig.align="center"}
...
Row or column annotation are often useful and can easy by added with the
`annotation_row` or `annotation_col` arguments, as described in the 
`pheatmap::pheatmap` method.

```{r demoPlot03, fig.align="center"}
